...
class CorpusHandler:
    ...
    def init_corpus_params(self, corpus_root, pattern):
        ...
        for file in ai_files:
            soup = BeautifulSoup(self.wordlists.raw(file), "lxml")    #get raw content of file

            for element in soup.find_all(['script', 'style']):      #remove script and style tags
                element.decompose()
...
if __name__ == "__main__":
    ...
    corpus_handler.dump_to_arff('A1D3_saimouli.arff', frequency_distribution, 'Frequency Distribution', ['Token', 'Count'])
    ...
    print "Classes : Artificial Intelligence (AI)/ Decision Tree (DT) / Ensemble Learning (EL)"

    while(True):
        input = raw_input("Enter something :")
        print "Result : ",classifier.classify(corpus_handler.feature_extractor(word_tokenize(input)))
